...
from model.networks import NLayerDiscriminator, PixelDiscriminator, SwinTransformer_Backbone
import functools
import sys
from collections import OrderedDict
import os
import torch.nn as nn
from model.config_su import get_config_or
sys.path.append("..")
...
class LPDGAN(nn.Module):
    ...
    def set_input(self, input):
        ...
        self.real_A1 = input['A1'].unsqueeze(0).to(self.device)
        self.real_B1 = input['B1'].unsqueeze(0).to(self.device)
        self.real_A2 = input['A2'].unsqueeze(0).to(self.device)
        self.real_B2 = input['B2'].unsqueeze(0).to(self.device)
        self.real_A3 = input['A3'].unsqueeze(0).to(self.device)
        self.real_B3 = input['B3'].unsqueeze(0).to(self.device)
