...
def make_divisible(x, divisor):
    ...
    new_list = ()
    for i in x:
        tmp = math.ceil(i / divisor) * divisor
        new_list = new_list + (tmp,)
    # print(new_list)
    return new_list
...
def save_one_box(xyxy, im, file='image.jpg', gain=1.02, pad=10, square=False, BGR=False, save=True):
    ...
    :params im: 原图片 需要裁剪的框从这个原图上裁剪  nparray  (1160, 720, 3)
    :params file: runs/detect/exp24/crops/LicencePlate/1.jpg
    ...
    if save:
        # file: WindowsPath('L:/_DATASET/output/exp5/crops/LicencePlate/00205459770115-90_85-352&516_448&547-444&547_368&549_364&517_440&515-0_0_22_10_26_29_24-128-7.jpg')
        # 保存剪切的目标框
        # print(file)
        RENAME = True
        if RENAME:
            lp = return_ccpd_info(Path(file).stem)   # '皖AYL250'    # Path(file).stem: '00205459770115-90_85-352&516_448&547-444&547_368&549_364&517_440&515-0_0_22_10_26_29_24-128-7'
            lp = f"{lp}_{Path(file).stem}"          # 皖AYL250_00205459770115-90_85-352&516_448&547-444&547_368&549_364&517_440&515-0_0_22_10_26_29_24-128-7
            Dir = Path(file).absolute().parent      # WindowsPath('L:/_DATASET/output/exp5/crops/LicencePlate')
            if not Dir.exists():
                os.makedirs(Dir)
                # print("""创建文件夹: %s""" % file)
            file = Path(Dir,lp).with_suffix('.jpg')    # WindowsPath('L:/_DATASET/output/exp5/crops/LicencePlate/皖AYL250.jpg')
        else:
            lp = Path(file).stem   # '皖AYL250'    # Path(file).stem: '00205459770115-90_85-352&516_448&547-444&547_368&549_364&517_440&515-0_0_22_10_26_29_24-128-7'
            Dir = Path(file).absolute().parent      # WindowsPath('L:/_DATASET/output/exp5/crops/LicencePlate')
            if not Dir.exists():
                os.makedirs(Dir)
                # print("""创建文件夹: %s""" % file)
            print(file)
            file = Path(Dir,lp).with_suffix('.jpg')

        # print("file:", file)
        # cv2.imwrite(str(increment_path(file, mkdir=True).with_suffix('.jpg')), crop)
        cv2.imencode('.jpg', crop)[1].tofile(file)
